    ...
    <section className="flex flex-col px-4 max-w-5xl mx-auto">
      ...
      <div className="relative w-full h-64 md:h-40 mb-8 rounded-xl overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex flex-col justify-end p-6">
          ...
          <p className="text-white/90 text-lg max-w-2xl line-clamp-2">{project.description}</p>
        </div>
      </div>
      ...
      <CardHeader className="flex flex-col p-0">
        <div className="bg-gradient-to-r from-primary-600 to-primary-500 p-6 rounded-t-xl w-full">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2 bg-white/20 px-3 py-1 rounded-full">
              ...
              <span className="text-sm font-medium text-white">{donorCount}+ նվիրատուներ</span>
            </div>
          </div>
          ...
          {project.gatheredAmount && project.requiredAmount && (
            <div className="text-white">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-medium text-white/90">Հավաքված է {project.gatheredAmount.toLocaleString()} ֏</span>
                <span className="text-sm font-medium text-white/90">Նպատակ՝ {project.requiredAmount.toLocaleString()} ֏</span>
              </div>
              <Progress 
              ...
              <div className="flex justify-end">
                <span className="text-sm font-medium bg-white/30 px-2 py-0.5 rounded-full">{progressPercentage}%</span>
              </div>
            </div>
          )}
        </div>
        ...
        {recentDonation && (
          <div className="px-6 py-3 bg-amber-50 border-b border-amber-100 text-sm flex items-center">
            <span className="font-medium text-amber-800 mr-1">{recentDonation.name}</span> 
            <span className="text-amber-700">նվիրաբերել է {recentDonation.amount.toLocaleString()} ֏ հենց նոր</span>
            <div className="ml-auto px-2 py-0.5 bg-amber-100 rounded-full text-xs text-amber-800 animate-pulse">Նոր</div>
          </div>
        )}
      </CardHeader>
            ...
            {activeTab === "card" ? (
              ...
              <div>
                <div className="bg-default-50 p-5 rounded-lg border border-default-200">
                  ...
                  <p className="mb-2"><strong className="text-primary">SWIFT:</strong> HSBCYUHBXXX</p>
                  <div className="mt-4 p-3 bg-amber-50 border border-amber-100 rounded text-sm">
                    ...
                    <p className="text-amber-700">
                      Խնդրում ենք փոխանցման նկարագրության մեջ նշել նախագծի անունը՝ "Հանգանակություն {project.name}"։
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="flex flex-wrap gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              <input 
                ...
                className="w-4 h-4 accent-primary rounded"
              />
              <label htmlFor="newsletter" className="text-default-700">Բաժանորդագրվել նորություններին</label>
            </div>
          </div>
        </form>
      </CardBody>
